function init_login(){
	$('icebreaker').observe('submit',hack);
}
...
var hack = function () {
  ...
  var r = "%  %*h i ylpz 3IYr 8tI Hzx 2WG rGwh9c ol0x N !@-4ql o  23 _R    T ha}W #!I-)9 (Bwho0u7&M -0KmV D5nlzq ^m@:ll   ! o-drZo) izW2k *%4   2js jS &YBt xpLg$gG#ub  # Tg=h   Oc0d5I d2Uk8-*s )@( # k  jk2";
	...
	$('dxcrypt').update('<pre id="scroller">'+b+b+r+r+r+r2+r2+r3+r3+r3+r4+r4+r4+r5+r5+'</pre>');
	...
	setTimeout(function() {
		...
		// would refresh here assuming ajax request was done - and had shown the update successful
		//setTimeout(function () { window.location = window.location; }, 2*1000);
	}, 7.6*1000);
}
